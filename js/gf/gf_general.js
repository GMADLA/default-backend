// JavaScript Document
function is_empty(testVar) {
	if (typeof testVar == 'undefined') return true;
...
var statesRequiringReload = new Array('NY','MT','UT','MA','WA','AL','DC','GA','MO','MS','OK','SC','TN');
...
function checkStateAndShowForm(showForm) {
	...
	if ( getStateValue != '') {
		...
		if(hiddenForm) {
	  		...
	  		// record ga event show address form here
    		if('function'==typeof(ga)) {
    			ga('send', {
    				hitType: 'event',
    				eventCategory: 'InquiryForm',
    				eventAction: 'gmadInquiryFormShowAddress',
    				eventLabel: 'formShown'
    			});
    		}	
		}
...
function __checkStateAndShowForm(showForm, prevStateSelected, currentStateSelected) {
	var optimizeForUX = true;

	if (is_empty(currentStateSelected)) {
		return ;
	}

	var stateException = determinePageReloadByState(currentStateSelected, prevStateSelected);
	var hiddenForm = $('.inquire_form_input_row').last().is(':hidden');
	if(hiddenForm) {
  		if('function'==typeof(recordEventShowLongForm)) {
  			recordEventShowLongForm();
	...
	if ( stateException ) {
		if (optimizeForUX) {
			...
			xdat.value = $('#inquiryForm').serialize();
			$('#stateForm').append(xdat);
			$('#stateForm').find('input[name=state]').val(currentStateSelected);
		}
		$('#stateForm').submit();
		return true;
	} else {
		postStateByAjax(currentStateSelected);
		if (showForm==true) {
            showStateForm();
        }
	}
...
function __determinePageReloadByState( currentStateSelected, prevStateSelected) {
	if (currentStateSelected==prevStateSelected) return false;
	
	if (statesRequiringReload.indexOf(currentStateSelected) >-1 ) {
		return true;
	}
	if (statesRequiringReload.indexOf(prevStateSelected) >-1 ) {
		return true;
	}
                ...
                if (typeof result === 'object' && result.status=='success' && result.region!='') {
            		...
            		if('function'==typeof(ga)) {
            			ga('send', {
            				hitType: 'event',
            				eventCategory: 'InquiryForm',
            				eventAction: 'prepop_state_by_ip',
            				eventLabel: result.region
            			});
            		}	                    
