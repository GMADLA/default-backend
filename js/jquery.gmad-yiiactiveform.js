...
;(function($) {
	...
	$.fn.yiiactiveform = function(options) {
		return this.each(function() {
			...
			$.each(settings.attributes, function(i, attribute) {
				...
				// Hook into yiiactiveform validation by allowing us to trigger validation via this custom event name, in addition to adding in extra validation
				$('#'+attribute.inputID, $form).bind('gmadYiiValidate', function(e, params){
					try {
						var overrideClientValidationCallback=undefined;
						var runInAddition=true;
						if (params!==undefined) {
							if ( params.clientValidate!==undefined) {
								overrideClientValidationCallback = params.clientValidate
							}
							if (params.runInAddition!==undefined) {
								runInAddition=params.runInAddition;
							}
						}
						$.fn.yiiactiveform.clientValidateAttribute($form, attribute, {}, function(data) {
								var hasError=false;
								hasError = $.fn.yiiactiveform.updateInput(attribute, data, $form);
							}, 
							overrideClientValidationCallback, runInAddition
						);
					} catch (e) {}
				});
	...
	/**
	 * We want to flush this out more, make it more powerful soon. For now, allow extra client validation to happen by passing it. 
	 */
	$.fn.yiiactiveform.clientValidateAttribute = function(form, attribute, messages, successCallback, clientValidateOverrideCallback, runCallbackInAddition) {
		var $form = $(form);
		var settings = $form.data('settings');
		var msg = [];
		var runOriginal=true;
		if (clientValidateOverrideCallback!==undefined) {
			if (runCallbackInAddition!==undefined) {
				runOriginal=runCallbackInAddition;
			}
			var value = $('#'+attribute.inputID, $form).val();
			clientValidateOverrideCallback(value, msg, attribute);
			if (msg.length) {
		...
		if (runOriginal ) {
			var value = $('#'+attribute.inputID, $form).val();
			attribute.clientValidation(value, msg, attribute);
			if (msg.length) {
				messages[attribute.id] = msg;
			}
		}
